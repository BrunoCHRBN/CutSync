...
import { isValidClientName, isValidClientPhone, normalizeClientName, normalizeClientPhone } from '../../src/utils/client-profile';
...
test('mantém Configurações como destino principal do cliente', () => {
  const shell = readSource('src/components/layout/ClientShell.tsx');
  const route = readSource('src/app/(client)/preferences.tsx');
...
test('protege a vitrine contra unidades não publicadas e mantém retry real', () => {
  const explore = readSource('src/components/screens/ExploreExperience.tsx');
