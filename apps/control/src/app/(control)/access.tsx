...
const roleOptions: {
  ...
  },
  {
    role: 'SaaS_Owner',
    label: 'SaaS_Owner',
    detail: 'Administra o Control e pode delegar ou revogar acessos.',
  },
];
    ...
    >
      ...
      <ControlCard style={styles.searchCard} testID="control-access-search">
        ...
        <View style={[styles.searchRow, mobile && styles.stack]}>
          <ControlField
            ...
            editable={!searching && !mutating && !pending}
            inputMode="email"
            label="E-mail da conta"
            onChangeText={setEmail}
            onSubmitEditing={() => {
              if (email.trim()) void searchProfile();
            }}
          ...
          <ControlButton
            busy={searching}
            disabled={!email.trim() || mutating || Boolean(pending)}
            label="Buscar conta"
      ...
      {pending ? (
        <ControlConfirmPanel
          ...
          description={
            pending.kind === 'revoke'
              ? 'O usuário perderá o acesso ao CutSync Control. Esta ação ficará registrada na auditoria.'
        ...
        >
          ...
          {target.intent !== 'revoke' ? (
            ...
            <ControlNotice
              message="A pessoa deixará de acessar todos os módulos do Control. As demais aplicações CutSync não serão afetadas."
              tone="warning"
            />
          )}
