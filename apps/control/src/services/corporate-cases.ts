...
export async function getCorporateCasesReadContext(): Promise<CorporateCasesReadContext> {
  const result = await supabase.rpc('get_corporate_cases_read_context');
...
export async function getCorporateCaseRuntimeAdministrationContext(
  ...
  const result = await supabase.rpc('get_corporate_case_runtime_administration_context', {
...
export async function setCorporateCaseRuntimeSettings(input: {
  ...
  const result = await supabase.rpc('set_corporate_case_runtime_settings', {
...
export async function listCorporateCaseTypes(): Promise<CorporateCaseType[]> {
  const result = await supabase.rpc('list_corporate_case_types');
...
export async function listCorporateCases(options: {
  ...
  const result = await supabase.rpc('list_corporate_cases', {
    target_view: view,
    target_status: options.status ?? undefined,
    target_limit: limit,
    target_cursor_updated_at: options.cursor?.timestamp,
    target_cursor_id: options.cursor?.id,
  });
...
export async function getCorporateCaseDetail(caseId: string): Promise<CorporateCaseDetail> {
  ...
  const result = await supabase.rpc('get_corporate_case_detail', { target_case_id: caseId });
...
export async function getCorporateCaseActionContext(
  ...
  if (!caseId.trim()) throw new CorporateCasesError('Informe um chamado válido.');
  const result = await supabase.rpc('get_corporate_case_action_context', { target_case_id: caseId.trim() });
...
export async function listCorporateCaseApprovalCandidates(input: {
  ...
  const result = await supabase.rpc('list_corporate_case_approval_candidates', {
...
export async function getCorporateCaseApprovalContext(
  ...
  if (!caseId.trim()) throw new CorporateCasesError('Informe um chamado válido.');
  const result = await supabase.rpc('get_corporate_case_approval_context', {
...
export async function decideCorporateCaseApproval(input: {
  ...
  const result = await supabase.rpc('decide_corporate_case_approval', {
    target_case_id: input.caseId.trim(),
    target_task_id: input.taskId.trim(),
...
export async function getCorporateCaseFulfillmentContext(
  ...
  if (!caseId.trim()) throw new CorporateCasesError('Informe um chamado válido.');
  const result = await supabase.rpc('get_corporate_case_fulfillment_context', {
    target_case_id: caseId.trim(),
  });
...
export async function listCorporateCaseFulfillmentQueue(options: {
  ...
  const limit = validateLimit(options.limit ?? 50);
  const result = await supabase.rpc('list_corporate_case_fulfillment_queue', {
    target_priority: options.priority ?? undefined,
    target_sla_state: options.slaState ?? undefined,
    target_attempt_state: options.attemptState ?? undefined,
    target_limit: limit,
    target_cursor_due_at: options.cursor?.dueAt,
    target_cursor_id: options.cursor?.id,
  });
...
export async function claimCorporateCaseFulfillment(input: {
  ...
  const result = await supabase.rpc('claim_corporate_case_fulfillment', {
    target_case_id: input.caseId.trim(),
    target_task_id: input.taskId.trim(),
    target_expected_case_version: input.expectedCaseVersion,
    target_expected_task_version: input.expectedTaskVersion,
...
export async function executeCorporateAccessFulfillment(input: {
  ...
  const result = await supabase.rpc('execute_corporate_access_fulfillment', {
    target_case_id: input.caseId.trim(),
    target_task_id: input.taskId.trim(),
    target_expected_case_version: input.expectedCaseVersion,
    target_expected_task_version: input.expectedTaskVersion,
...
export async function claimCorporateCaseTask(input: {
  ...
  const result = await supabase.rpc('claim_corporate_case_task', {
    target_case_id: input.caseId.trim(),
    target_task_id: input.taskId.trim(),
    target_expected_case_version: input.expectedCaseVersion,
    target_expected_task_version: input.expectedTaskVersion,
...
export async function advanceCorporateCaseTask(input: {
  ...
  const result = await supabase.rpc('advance_corporate_case_task', {
    target_case_id: input.caseId.trim(),
    target_task_id: input.taskId.trim(),
    target_expected_case_version: input.expectedCaseVersion,
    target_expected_task_version: input.expectedTaskVersion,
...
export async function listCorporateNotifications(options: {
  ...
  const limit = validateLimit(options.limit ?? 50);
  const result = await supabase.rpc('list_corporate_notifications', {
    target_unread_only: options.unreadOnly ?? false,
    target_limit: limit,
    target_cursor_created_at: options.cursor?.timestamp,
    target_cursor_id: options.cursor?.id,
  });
...
export async function listCorporateAccessRequestProfiles(): Promise<CorporateAccessRequestProfile[]> {
  const result = await supabase.rpc('list_corporate_access_request_profiles');
...
export async function findCorporateCaseIdentityByEmail(
  ...
  const result = await supabase.rpc('find_corporate_case_participant_by_email', {
    target_email: email,
  });
...
export async function createCorporateAccessCase(input: {
  ...
  // Supabase's generated function metadata does not express nullable SQL
  // parameters unless they also have defaults. Preserve the runtime NULL for
  // target_valid_until while keeping every other RPC argument fully generated.
  const validUntil: string | null = input.validUntil ?? null;
  const result = await supabase.rpc('create_corporate_access_case', {
    target_beneficiary_profile_id: beneficiaryProfileId,
    target_requested_profile_key: requestedProfileKey,
    target_action: input.action,
    target_source_profile_key: input.sourceProfileKey?.trim() || '',
    target_valid_until: validUntil as string,
