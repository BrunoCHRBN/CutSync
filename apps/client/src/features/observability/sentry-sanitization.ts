...
const CLIENT_DYNAMIC_ROUTE_SEGMENTS: Record<string, '[id]' | '[slug]'> = {
  appointments: '[id]',
  booking: '[slug]',
  establishments: '[slug]',
  support: '[id]',
};
const CLIENT_STATIC_CHILD_ROUTES = new Set(['index', 'new']);
...
export const sanitizeSentryRoute = (value: string) => {
  ...
  const sanitizedSegments = segments.map((segment, index) => {
    ...
    const routePlaceholder = previousSegment
      ? CLIENT_DYNAMIC_ROUTE_SEGMENTS[previousSegment]
      : undefined;
    if (routePlaceholder && !CLIENT_STATIC_CHILD_ROUTES.has(segment.toLowerCase())) {
