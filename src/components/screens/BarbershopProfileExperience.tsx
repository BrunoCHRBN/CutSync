...
export const BarbershopProfileExperience = () => {
  ...
  if (loading) return <ScreenBackground testID="barbershop-profile-loading" style={styles.center}><ActivityIndicator color={colors.brand} size="large" /></ScreenBackground>;
  if (!barbershop) return <ScreenBackground testID="barbershop-profile-not-found" style={styles.center}><EmptyState testID="barbershop-profile-error" title="Barbearia não encontrada" description="Este perfil pode ter sido removido ou ainda não foi sincronizado." icon={<Store color={colors.brand} size={22} />} action={<AppButton label="Voltar para explorar" testID="barbershop-profile-error-back-button" onPress={goBack} />} /></ScreenBackground>;
  ...
  return (
    <ScreenBackground testID="barbershop-profile-screen">
      <View style={styles.topbar}>
        <Pressable testID="barbershop-profile-back-button" onPress={goBack} style={styles.backButton}><ArrowLeft color={colors.text} size={18} /></Pressable>
        <Text testID="barbershop-profile-topbar-title" numberOfLines={1} style={styles.topbarTitle}>{barbershop.name}</Text>
        <AppButton label="Agendar" testID="barbershop-profile-topbar-book-button" onPress={() => router.push(`/(client)/booking?barbershopId=${barbershopId}`)} style={[styles.topbarBook, { backgroundColor: accent, borderColor: accent }]} />
      </View>

      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={[styles.hero, isWide && styles.heroWide]}>
          <View style={[styles.heroVisual, { backgroundColor: `${accent}16`, borderColor: `${accent}44` }]}>
            {barbershop.logoUrl ? <Image testID="barbershop-profile-logo" source={{ uri: barbershop.logoUrl }} resizeMode="cover" style={styles.heroLogo} /> : <Text style={[styles.heroMonogram, { color: accent }]}>{barbershop.name.charAt(0).toUpperCase()}</Text>}
          ...
          <View style={styles.heroCopy}>
            ...
            <Text testID="barbershop-profile-description" style={styles.description}>{barbershop.description || 'Este estabelecimento ainda não adicionou uma descrição.'}</Text>
        ...
        <View style={styles.section}>
          ...
          {barbers.length === 0 ? <EmptyState testID="barbershop-team-empty" title="Equipe em atualização" description="Os profissionais aparecerão aqui em breve." icon={<UsersRound color={accent} size={22} />} /> : (
            <View testID="barbershop-team-grid" style={styles.cardsGrid}>{barbers.map((barber) => <AppCard key={barber.id} testID={`barbershop-professional-${barber.id}`} style={styles.professionalCard}><View style={[styles.avatar, { backgroundColor: `${accent}18` }]}>{barber.avatarUrl ? <Image source={{ uri: barber.avatarUrl }} style={styles.avatarImage} /> : <UserRound color={accent} size={22} />}</View><Text style={styles.professionalName}>{barber.name}</Text><Text style={styles.professionalRole}>{barber.role === 'admin' ? 'Proprietário' : 'Barbeiro'}</Text></AppCard>)}</View>
          )}
        </View>

        <View testID="barbershop-booking-cta" style={[styles.cta, { borderColor: `${accent}55`, backgroundColor: `${accent}12` }]}>
          <View style={styles.ctaCopy}><Text style={styles.ctaEyebrow}>PRONTO PARA MUDAR O VISUAL?</Text><Text style={styles.ctaTitle}>Escolha serviço, profissional e horário.</Text></View>
          <AppButton label="Agendar agora" testID="barbershop-profile-book-button" onPress={() => router.push(`/(client)/booking?barbershopId=${barbershopId}`)} icon={<ArrowRight color={colors.ink} size={17} />} style={{ backgroundColor: accent, borderColor: accent }} />
        </View>
      </ScrollView>
    </ScreenBackground>
  );
...
const InfoItem = ({ icon, label, value, testID }: { icon: React.ReactNode; label: string; value: string; testID: string }) => <View testID={testID} style={styles.infoItem}><View style={styles.infoIcon}>{icon}</View><View style={styles.infoCopy}><Text style={styles.infoLabel}>{label}</Text><Text style={styles.infoValue}>{value}</Text></View></View>;
