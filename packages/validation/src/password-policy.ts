...
export const PASSWORD_RULES: PasswordRule[] = [
  ...
  { id: 'symbol', label: 'Um símbolo especial', isMet: (password) => /[^A-Za-z0-9]/.test(password) },
];

export const isStrongPassword = (password: string) => PASSWORD_RULES.every((rule) => rule.isMet(password));
