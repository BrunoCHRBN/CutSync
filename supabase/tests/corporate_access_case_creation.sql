...
BEGIN
  ...
  END IF;
  IF (SELECT count(*) FROM public.corporate_case_access_requests WHERE case_id = created_case_id) <> 1 THEN
    RAISE EXCEPTION 'FAIL: structured access projection missing';
  END IF;
  IF (SELECT count(*) FROM public.corporate_case_participants WHERE case_id = created_case_id) <> 3 THEN
    RAISE EXCEPTION 'FAIL: requester, beneficiary and observer were not registered';
  END IF;
  IF (SELECT count(*) FROM public.corporate_case_tasks WHERE case_id = created_case_id) <> 1 THEN
    RAISE EXCEPTION 'FAIL: creation must materialize only the active triage task';
  END IF;
  IF (SELECT count(*) FROM public.corporate_case_sla_instances WHERE case_id = created_case_id) <> 2 THEN
    RAISE EXCEPTION 'FAIL: case and task SLA instances missing';
  END IF;
  IF (SELECT count(*) FROM public.corporate_case_events WHERE case_id = created_case_id) <> 1 THEN
  ...
  IF (
    ...
    WHERE event.case_id = created_case_id
  ) <> 4 THEN
  ...
  IF (
    ...
    JOIN public.corporate_case_events AS event ON event.id = notification.event_id
    WHERE event.case_id = created_case_id AND outbox.channel = 'email'
  ) <> 1 THEN
  ...
  repeated := public.create_corporate_access_case(
    ...
    'grant',
    NULL,
    (SELECT requested_valid_until FROM public.corporate_case_access_requests WHERE case_id = created_case_id),
    'Necessidade financeira válida para conciliação e acompanhamento operacional.',
    ARRAY['b0000000-0000-4000-8000-000000000003']::uuid[],
    'b1000000-0000-4000-8000-000000000001'
  );
