...
CREATE INDEX schedule_blocks_establishment_period_idx
  ON public.schedule_blocks (establishment_id, starts_at, ends_at)
  WHERE deleted_at IS NULL;
CREATE INDEX schedule_blocks_professional_period_idx
...
CREATE FUNCTION public.get_schedule_blocks(
...
CREATE FUNCTION public.delete_schedule_block(target_block_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public
...
CREATE FUNCTION public.compute_available_slots(
  target_establishment_id uuid,
  target_professional_id uuid,
